...
class GraphIntegrator:
    ...
    def create_nodes(self, json_graph: dict) -> List[str]:
        ...
        for node in json_graph["nodes"]:
            ...
            query = f'CREATE (n:{node["label"]} {{name: "{node["name"]}", doc_id: "{node["doc_id"]}", description: "{node["description"]}"}}) '
    ...
    def create_relationships(self, json_graph: dict) -> list:
        ...
        for rel in json_graph["relationships"]:
            property_statements = ' '.join(
                [
                    f'SET r.{key.replace(" ", "_")} = "{value}"' if key != "embeddings" else f'SET r.{key.replace(" ", "_")} = "{GraphIntegrator.transform_embeddings_to_str_list(value)}"'
            ...
            # query = (f'MATCH (n {{name: "{rel["startNode"]}"}}), (m {{name: "{rel["endNode"]}"}}) '
            #          f'MERGE (n)-[r:Relationship {{doc_id: "{rel["doc_id"]}", start_node: "{rel["startNode"]}", '
            #          f'end_node: "{rel["endNode"]}", relation: "{rel["name"]}"}}]->(m) ')
            query = (f'MATCH (n {{name: "{rel["startNode"]}"}}), (m {{name: "{rel["endNode"]}"}}) '
                      f'MERGE (n)-[r:Relationship {{doc_id: "{rel["doc_id"]}", relation: "{rel["name"]}"}}]->(m) ')
            rels.append(query)
