...
class iText2KG:
    ...
    def build_graph(self, sections:List[str], doc_id:str, dic: List[dict], existing_global_entities:List[dict]=None, existing_global_relationships:List[dict]=None, ent_threshold:float = 0.7, rel_threshold:float = 0.7):
        """
        ...
        """
        print("[INFO] Extracting Entities from the Document", 1)
        global_entities = self.ientities_extractor.extract_entities(context=sections[0], doc_id=doc_id, dic=dic)
        print("[INFO] Extracting Relations from the Document", 1)
        global_relationships = self.irelations_extractor.extract_relations(context=sections[0], entities = list(map(lambda w:w["name"], global_entities)), doc_id=doc_id)
        ...
        while isolated_entities:
            ...
            corrected_relations = self.irelations_extractor.extract_relations_for_isolated_entities(context=sections[0], isolated_entities=list(map(lambda w:w["name"],isolated_entities)), doc_id=doc_id, local_non_isolated_entities=list(map(lambda w:w["name"],global_entities)))
        ...
        for i in range(1, len(sections)):
            print("[INFO] Extracting Entities from the Document", i+1)
            entities = self.ientities_extractor.extract_entities(context= sections[i], doc_id=doc_id, dic=dic)

            processed_entities, global_entities = self.matcher.process_lists(list1 = entities, list2=global_entities, for_entity_or_relation="entity", threshold=ent_threshold)

            # relationships = relationship_extraction(context= sections[i], entities=list(map(lambda w:w["name"], processed_entities)))
            print("[INFO] Extracting Relations from the Document", i+1)
            relationships = self.irelations_extractor.extract_relations(context= sections[i], entities=list(map(lambda w:w["name"], processed_entities)), doc_id=doc_id)
            ...
            while isolated_entities:
                print("[INFO] The isolated entities are ", isolated_entities)
                corrected_relations = self.irelations_extractor.extract_relations_for_isolated_entities(context=sections[i], isolated_entities=list(map(lambda w:w["name"],isolated_entities)), doc_id=doc_id, local_non_isolated_entities=list(map(lambda w:w["name"],processed_entities)))
                matched_corrected_relationships, _ = self.matcher.process_lists(list1 = corrected_relations, list2=global_relationships, for_entity_or_relation="relation", threshold=rel_threshold)
