...
class DataHandler:
    ...
    def add_embeddings_as_property_batch_ent(self, embeddings_function, items: List[dict], doc_id: str, dic: List[dict], property_name="properties", embeddings_name="embeddings", item_name_key="name", embeddings=True, doc_name='doc_id',dict_name='description'):
        """
        ...
        if embeddings:
            ...
            name_to_description = {item['name']: item['description'] for item in dic}
            descriptions = [name_to_description.get(name) for name in item_names]

            # Use zip to efficiently assign embeddings to each item
            items = [
                {**item, property_name: {embeddings_name: embedding}, doc_name: doc_id, dict_name: des}
                for item, embedding, des in zip(items, all_embeddings, descriptions)
            ]

        return items


    def add_embeddings_as_property_batch(self, embeddings_function, items: List[dict], doc_id: str, property_name="properties", embeddings_name="embeddings", item_name_key="name", embeddings=True, doc_name='doc_id'):
        """
        Add embeddings as a property to a list of dictionaries (items), such as entities or relations.
        ...
        if embeddings:
            ...
            # Use zip to efficiently assign embeddings to each item
            items = [
                {**item, property_name: {embeddings_name: embedding}, doc_name:doc_id}
    ...
    def process(self, data: dict, data_type: Literal['entity', 'relation']) -> dict:
        ...
        if data_type == 'relation':
            ...
            data["name"] = re.sub(r'[^a-zA-Z0-9\u4e00-\u9fff]', '_', data["name"]).replace("&", "and")
