...
class iEntitiesExtractor():
    ...
    def extract_entities(self, context: str, doc_id: str, dic: List[dict], embeddings: bool = True, property_name = "properties", entity_name_key = "name", doc_name = 'doc_id', dict_name='description'):
        """
        ...
        if "entities" not in entities.keys() or entities == None:
            ...
            self.extract_entities(context=context, embeddings=embeddings, property_name=property_name, entity_name_key=entity_name_key, dic=dic, doc_id=doc_id)
        
        return self.data_handler.add_embeddings_as_property_batch_ent(embeddings_function=lambda x:self.langchain_output_parser.calculate_embeddings(x),
